...
function HMACForm(props) {
    ...
    return (
        <>
            ...
            <ControlGroup label="HMAC Secret" tooltip="This will be used by the Webhook receiver to verify the request">
                <Text
            ...
            <ControlGroup label="Hash Function">
                <Select defaultValue="sha1" value={props.cred.hmac_hash_function} onChange={handleChangeHashFunction}>
                    <Select.Option label="SHA1" value="sha1" />
                    <Select.Option label="SHA256" value="sha256" />
                    <Select.Option label="SHA512" value="sha512" />
                </Select>
            </ControlGroup>
            <ControlGroup label="Digest Type" tooltip="This is how the HMAC signature is encoded in the HTTP header. I usually see this base64-encoded, but I've added an option to send as a hex digest too in case a user needs it.">
            ...
            <ControlGroup label="HMAC Secret Header" tooltip="The HTTP header that the HMAC signature will be sent in" help="e.g. X-Sign">
                <Text
                    value={props.cred.hmac_sig_header}
                    autoComplete="off"
                    onChange={handleChangeSigHeader} />
            </ControlGroup>
            <ControlGroup label="HMAC Timestamp Header" tooltip="The HTTP header that the HMAC timestamp will be sent in" help="Leave this blank to entirely skip using timestamp to compute the HMAC signature. Some services use timestamp, some don't.">
                <Text
